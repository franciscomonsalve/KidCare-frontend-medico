...
import { parseResumen } from '../utils/parseResumen'
...
function calcularHoraGenerado(expiracionISO) {
  if (!expiracionISO) return '—'
  const d = new Date(new Date(expiracionISO).getTime() - 20 * 60 * 1000)
  const hoy = new Date()
  const hora = d.toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })
  return d.toDateString() === hoy.toDateString() ? `hoy ${hora}` : hora
}
...
export default function BitacoraPage() {
  ...
  // Redirigir si no hay estado (acceso directo a /bitacora sin verificar)
  useEffect(() => {
    if (!menorId) navigate('/error', { state: { tipo: 'expired' } })
  }, [])

  // Timer expirado → redirigir tras 3 segundos mostrando el banner
  useEffect(() => {
    if (!expirado) return
    const t = setTimeout(() => {
      navigate('/error', { state: { tipo: 'expired' } })
    }, 3000)
    return () => clearTimeout(t)
  }, [expirado])

  // Resumen: usar el que ya vino con verificarToken si está disponible
  useEffect(() => {
    if (!menorId) return
    if (resumenVerif) {
      const parsed = parseResumen(resumenVerif)
      setResumen({ resumenDisponible: true, resumen: resumenVerif, ...(parsed || { motivo: resumenVerif }) })
      setLoadingR(false)
      return
    }
    // Fallback: llamar al endpoint de historial
    getResumen(menorId)
      .then(d => {
        const parsed = parseResumen(d.resumen)
        setResumen({ ...d, ...(parsed || {}) })
        setLoadingR(false)
      })
      .catch(() => { setResumen({ resumenDisponible: false }); setLoadingR(false) })
  }, [menorId, resumenVerif])

  // Observaciones del chatbot
  useEffect(() => {
    if (!menorId) return
  ...
  // Hora de generación: prefiere el valor del backend, sino calcula desde expiracion
  const horaGeneradoDisplay = horaGenerado
    ? `hoy ${formatHoraCorta(horaGenerado)}`
    : calcularHoraGenerado(expiracion)
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#f0f4f8' }}>
      <style>{`
        ...
        @media(max-width:760px){
          ...
          .sidebar { order: -1; }
          .hero-name { font-size: clamp(24px, 6vw, 38px) !important; }
        }
      `}</style>
      ...
      {/* Sesión expirada — banner antes de redirect */}
      {expirado && (
        <div style={{
          ...
          ⛔ Sesión expirada · El enlace temporal ha vencido. Redirigiendo…
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div className="hero-name" style={{
            ...
            {nombreMenor || 'Paciente'}
            {edadMenor ? `, ${edadMenor} años` : ''}
          </div>

          {/* Subtítulo tutor / médico */}
          {(nombreTutor || nombreMedicoNav) && (
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,.55)', marginBottom: 0 }}>
              {nombreTutor && (
                <>Compartido por <strong style={{ color: 'rgba(255,255,255,.85)', fontWeight: 600 }}>{nombreTutor} (Tutor)</strong></>
              )}
              {nombreMedicoNav && (
                <> · Para <strong style={{ color: 'rgba(255,255,255,.85)', fontWeight: 600 }}>{nombreMedicoNav}</strong></>
              )}
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 20 }}>
            {[
              { text: `📅 Enlace generado: ${horaGeneradoDisplay}` },
              { text: `⏱️ Expira a las ${formatHoraCorta(expiracion)}` },
            ...
            <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: 18 }}>
              ...
              <div style={{ padding: 20 }}>
                ...
                <div style={{ marginBottom: 22 }}>
                  <SecLabel>📌 {resumen.evolucion ? 'Motivo de consulta' : 'Resumen clínico'}</SecLabel>
                  <div style={{ fontSize: 14, color: '#1e293b', lineHeight: 1.7 }}>
                    {resumen.motivo || resumen.resumen}
                  </div>
                </div>

                {/* Síntomas — solo si el parser los extrajo */}
                {(resumen.sintomas?.length > 0 || resumen.sintomasDesc) && (
                  <>
                    <Divider />
                    <div style={{ marginBottom: 22 }}>
                      ...
                      {resumen.sintomasDesc && (
                        <div style={{ fontSize: 14, color: '#1e293b', lineHeight: 1.7, marginTop: resumen.sintomas?.length ? 10 : 0 }}>
                          {resumen.sintomasDesc}
                        </div>
                      )}
                    </div>
                ...
                {/* Evolución — solo si el parser la extrajo */}
                {resumen.evolucion && (
                  <>
                    <Divider />
                    <div style={{ marginBottom: 22 }}>
                ...
                {/* Escala de malestar */}
                {resumen.escalaMalestar != null && (
                  <>
                    <Divider />
                    <div>
                      <SecLabel>😣 Escala de malestar — último registro</SecLabel>
                      <PainTrack value={resumen.escalaMalestar} />
                    </div>
                  </>
                )}
        ...
        <div className="sidebar">
          ...
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: 16 }}>
            ...
            <div style={{ padding: '0 18px' }}>
              {[
                { icon: '🧒', label: 'Nombre',             val: nombreMenor || '—' },
                { icon: '🎂', label: 'Edad',               val: edadMenor ? `${edadMenor} años` : '—' },
                { icon: '⚕️', label: 'Alergias',           val: '—' },
                { icon: '📋', label: 'Condiciones previas', val: '—' },
                { icon: '👩', label: 'Tutor',              val: nombreTutor || '—' },
              ].map((r, i, arr) => (
                <div key={i} style={{
          ...
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '0 18px' }}>
              {[
                { icon: '🩺', label: 'Médico',          val: nombreMedicoNav || '—' },
                { icon: '🕐', label: 'Enlace generado',  val: horaGeneradoDisplay },
                { icon: '⏱️', label: 'Expira a las',     val: formatHoraCorta(expiracion), timer: true },
                ...
                <div key={i} style={{
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#1e293b', marginTop: 2, lineHeight: 1.4 }}>
                      {r.timer
                        ? <>{r.val} &nbsp;<span style={{ color: '#94a3b8', fontWeight: 400 }}>({timerDisplay})</span></>
                        : r.val
                      }
                    </div>
                  </div>
